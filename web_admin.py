...
5. 📺 YouTube 解析指令：你已經具備讀取 YouTube 影片內容的能力。當老闆給你 YouTube 連結或要求總結影片時，請務必立刻調用 `summarize_youtube_video` 工具提取內容，絕對禁止回答「無法觀看」或「不具備能力」！
...
9. 🚨 拒絕延遲與空頭支票（極度重要）：身為 AI，你的系統架構屬於單次觸發機制。當任務涉及多個目標（如查多個網站），你必須在「同一次思考」中並行發出所有工具調用。絕對禁止先查第一個，然後說「請稍候，我現在去查第二個」！【⚠️ 特例豁免】：如果老闆明確要求「預約在未來的時間」(例如：今天 10:20、每天下午3點)，你必須立刻調用 `schedule_custom_task` 工具進行排程。這屬於合法操作，絕對不算空頭支票，請放心調用！
                            ...
                            try:
                                if fn_name in AGENT_TOOLS_REGISTRY:
                                    ...
                                    try:
                                        ...
                                        if isinstance(rj, dict):
                                            if rj.get("type") == "webpage_with_screenshot":
                                                temp_memory.append({"role": "tool", "tool_call_id": tc['id'], "name": fn_name, "content": f"文字內容：{rj.get('text', '')}"})
                                                ...
                                                temp_memory.append({"role": "tool", "tool_call_id": tc['id'], "name": fn_name, "content": rj.get("text", "成功擷取")})
                                    ...
                                    if not is_ss:
                                        ...
                                        temp_memory.append({"role": "tool", "tool_call_id": tc['id'], "name": fn_name, "content": tool_out})
                            except Exception as e:
