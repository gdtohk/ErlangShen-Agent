...
*   **🌐 網頁截圖與瀏覽 (New!)**：支援訪問網址並自動擷取 **即時截圖 (Screenshot)**，讓 AI 能結合視覺畫面分析網頁資訊。
*   **📺 YouTube 影片總結 (New!)**：提供網址即可自動提取影片字幕 (Transcript)，秒速總結影片精華。
*   **📑 工程文件解析 (PDF/Excel)**：支援直接讀取 PDF 規格書及 Excel BBS 鋼筋表，秒速歸納重點。
*   **📰 全球即時資訊 (RSS)**：內建新聞抓取通道，即時讀取全球頭條，絕對防封鎖。
*   **🧮 鋼筋重量精算**：內建專業工具，一鍵計算鋼筋重量 (d²/162.2)。
*   **⏰ 定時提醒與排程**：支援設定私人鬧鐘與每日天氣晨報。
