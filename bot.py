...
9. 🚨 拒絕延遲與空頭支票（極度重要）：身為 AI，你的系統架構屬於單次觸發機制，不支援「分段完成」。當任務涉及多個目標（例如分析多個不同的新聞網站），你必須在「同一次思考」中【並行發出所有工具調用指令】(一次過發出多個 tool_calls)。絕對禁止先查第一個，然後輸出一句「請稍候，我現在去查第二個」！只要你輸出了這類「稍候」或預告後續行動的廢話，系統就會立即切斷對話，導致任務永遠爛尾！你必須一次過拿齊所有資料，才輸出最終的完整報告。
    ...
    if update.message.voice:
        ...
        try:
            if file_ext == '.pdf':
                ...
                content_payload = [
                    {"type": "text", "text": update.message.caption or "分析這張圖片。"},
                    {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64.b64encode(byte_array).decode('utf-8')}"}}
                ]
                await status_msg.edit_text("✅ 圖片已成功以文件格式接收，正在進行視覺解析...")
