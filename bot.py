...
你具備語音、視覺、文件解析 (PDF/Excel)、網頁截圖、YouTube 字幕解讀能力。

【🚨 真理指令】：
1. 如果工具調用失敗，請老實告訴老闆，絕對禁止憑空編造！
2. 調用 browse_website 後，系統會為你注入網頁截圖，請務必進行視覺分析。
3. 調用 analyze_youtube_video 後，請根據獲取的字幕進行總結。
"""
...
async def daily_morning_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=chat_id, text=f"🌅 早晨{OWNER_NAME}！今日係 {date_str}。祝工作順利！")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.voice:
        ...
        try:
            ...
            content_payload = f"【問題】：{update.message.caption or '分析文件'}\n\n{extracted_content}"
                    ...
                    for tc in msg['tool_calls']:
                        ...
                        try:
                            ...
                            if rj.get("type") == "webpage_with_screenshot":
                                ...
                                user_memory[user_id].append({"role": "user", "content": [{"type": "text", "text": "請結合此網頁截圖分析。"}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{rj['image_base64']}"}}]})
                                is_ss = True
                        except: pass

                        if not is_ss:
                            tool_out = str(res)
                            if "❌" in tool_out: tool_out += " (獲取失敗，不可編造答案！)"
...
def main():
    ...
    app.job_queue.run_daily(daily_morning_report, datetime.time(hour=5, minute=30, tzinfo=ZoneInfo(TIMEZONE_STR)))
